...
void file_io::checkFiles(const String &inputFilePath, const String &outputFilePath) {
...
Buffer file_io::readFileToBuffer(const String &inputFile) {
    std::ifstream ifs(inputFile.c_str(), std::ios::binary | std::ios::ate);
    if (!ifs)
...
void file_io::writeToFile(const String &outputFile, const Packed &packed) {
    std::ofstream ofs(outputFile.c_str(), std::ios::out | std::ios::binary | std::ofstream::trunc);
    if (!ofs)
